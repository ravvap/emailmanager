...
public class DataConnectionServiceImpl implements DataConnectionService {
    ...
    private final TemplateUsageChecker templateUsageChecker;
    ...
    public void deleteConnection(Long id, String username) {
        DataConnection connection = repository.findByIdAndDeletedAtIsNull(id)
 		...
 		if (templateUsageChecker.isConnectionUsedByAnyTemplate(id)) {
    ...
    public List<DataConnectionDto> getActiveConnectionsForAuthor(String username) {
        // Was: `return null;` - the author-facing "pick an active, authorized connection"
        // flow had no working entry point at all. Resolve username -> app_user.id, then
        // delegate to the existing id-based query.
        Optional<AppUser> user = appUserRepository.findByUsernameIgnoreCase(username);
        if (user.isEmpty()) {
            log.warn("getActiveConnectionsForAuthor: no app_user found for username '{}'", username);
            return Collections.emptyList();
        }
        return getActiveConnectionsForAuthor(user.get().getId());
    }
    ...
    public ConnectionTestResultDto testConnection(DataConnectionDto dto) {
        ...
        String vaultCredentialRef = StringUtils.hasText(dto.getVaultCredentialRef())
                ? dto.getVaultCredentialRef()
                : location.getVaultCredentialRef();
